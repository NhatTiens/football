...
# PredictionAI Automatic Pipeline installer v1.7 - robust upcoming UI type cleanup
...
$stablePatch = Join-Path $PSScriptRoot 'predictionai-automatic-pipeline-v1.6-stable.patch'
$transformer = Join-Path $PSScriptRoot 'predictionai-automatic-pipeline-v1.7-transform.mjs'
...
try {
    ...
    try {
        ...
        Write-Host 'PREDICTIONAI_AUTOMATIC_PIPELINE_V1_7_LOCAL_TRANSFORM_PREFLIGHT_PASS'
    }
    finally {
        Pop-Location
    }
...
$manifest = [ordered]@{
    version = 'predictionai-automatic-pipeline-v1.6'
...
try {
    ...
    Write-Host 'PredictionAI automatic fixture/prediction/result/realtime pipeline V1.7 applied and full repository verify passed.'
