...
scientificRouter.get('/bets', async (request, response, next) => {
  try {
    ...
    const data = [...ledgerRows, ...shadowRows]
      ...
      .slice(0, limit);
    const ruleCompliantPaperRows = paperRows.filter(
      (row) =>
        historyOuRuleStatus({
          market: row.marketType,
          selection: row.selection,
    ...
    const legacyOuPaperProposals = paperRows.length - ruleCompliantPaperRows.length;    const settledPaperRows = ruleCompliantPaperRows.filter(
    ...
    response.json({
      ...
      summary: {
        paperProposals: ruleCompliantPaperRows.length,
        legacyOuPaperProposals,
        pendingPaperProposals: ruleCompliantPaperRows.filter((row) => row.status === 'PENDING_OUTCOME').length,
        invalidPaperProposals: ruleCompliantPaperRows.filter((row) => row.status === 'INVALID_DECISION_LINEAGE')
          .length,
