...
export async function settleOpenScientificPaperBets(input: {
  ...
  const decisionFixtureIds = [...new Set(decisions.map((row) => row.providerFixtureId))];
  const shadowFixtureIds = [
    ...new Set(
      shadowSnapshots
        .filter((row) => isTrackedPaperShadowRecommendation(row.analysisPayload))
        .map((row) => row.providerFixtureId),
    ),
  ];
  const fixtureCandidates = [...new Set([...decisionFixtureIds, ...shadowFixtureIds])];
  ...
  const alreadyTerminal = new Set(existingTerminalRows.map((row) => row.providerFixtureId));
  const fixtureIdsToFetch = fixtureCandidates
    .filter((providerFixtureId) => !alreadyTerminal.has(providerFixtureId))
