...
function fail(message) {
  throw new Error(`[automatic-pipeline-v1.7] ${message}`);
}
...
function removeTypeImportOnlyWhenUnused(text, typeName) {
  const importLine = new RegExp(`\\n\\s*${typeName},`);
  const candidate = text.replace(importLine, '');
  const stillUsed = new RegExp(`\\b${typeName}\\b`).test(candidate);
  return stillUsed ? text : candidate;
}

function assertUpcomingTypeContract(text) {
  for (const typeName of ['CurrentCompetitionGroup', 'PersonalRefreshResponse']) {
    const used = new RegExp(`\\b${typeName}\\b`).test(text);
    if (!used) continue;
    const imported = new RegExp(
      `import type \\{[\\s\\S]*?\\b${typeName}\\b[\\s\\S]*?\\} from '../lib/personal-types';`,
    ).test(text);
    if (!imported) fail(`upcoming type used without import (${typeName})`);
  }
}
...
async function updateUpcomingBoard() {
  ...
  if (text.includes(`// ${MARKER}_UPCOMING_UI`)) {
    assertUpcomingTypeContract(text);
    await writeSource(file, text);
    return;
  }
  ...
  // Remove legacy manual-selection state structurally before touching type imports.
  // The local frontend has evolved several times, so do not assume exact line wrapping.
  text = text.replace(
    /(?:^|\n)const GROUPS:\s*Array<\{[\s\S]*?\}>\s*=\s*\[[\s\S]*?\n\];\s*/m,
    '\n',
  );
  text = text.replace(
    /\n\s*const \[selectedGroups,\s*setSelectedGroups\]\s*=\s*useState<CurrentCompetitionGroup\[\]>\(\[[\s\S]*?\]\);\s*/m,
    '\n',
  );
  text = text.replace(
    /\n\s*const \[lastDiscovery,\s*setLastDiscovery\]\s*=\s*useState<PersonalRefreshResponse\['discovery'\]\s*\|\s*null>\([\s\S]*?\);\s*/m,
    '\n',
  );
  ...
  text = removeTypeImportOnlyWhenUnused(text, 'CurrentCompetitionGroup');
  text = removeTypeImportOnlyWhenUnused(text, 'PersonalRefreshResponse');
  assertUpcomingTypeContract(text);
...
console.log('PREDICTIONAI_AUTOMATIC_PIPELINE_V1_7_TRANSFORM_PASS');
