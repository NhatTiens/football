...
import {
  extractOuAuditFromDecisionPayload,
  replayOuBetHistoryRows,
} from './ou-history-replay.js';
...
function ledgerOuAudit(input: {
  ...
  if (!isHistoryOuMarket(input.market)) {
    return {
      sourcePredictionSelection: extractOuAuditFromDecisionPayload(decision.decisionPayload)?.predictionSelection ?? null,
      sourcePredictionLineValue: extractOuAuditFromDecisionPayload(decision.decisionPayload)?.predictionLineValue ?? null,
      sourcePredictionProbability: extractOuAuditFromDecisionPayload(decision.decisionPayload)?.predictionProbability ?? null,
