export const THREE_MARKET_CORE_VERSION = 'v7.3-ou-specialist-r3';
...
function calibrateExpectedGoalsToTotals(input: {
  homeExpectedGoals: number;
  awayExpectedGoals: number;
  rho: number;
  maximumGoals: number;
  targets: Partial<Record<ThreeMarketTotalLine, number>>;
  directWeight: number;
}): { homeExpectedGoals: number; awayExpectedGoals: number } {
  const available = THREE_MARKET_TOTAL_LINES.filter((line) => Number.isFinite(input.targets[line]));
  if (available.length === 0) {
    return { homeExpectedGoals: input.homeExpectedGoals, awayExpectedGoals: input.awayExpectedGoals };
  }
  const baseGrid = buildScoreGrid(input).scoreGrid;
  const blendedTargets = new Map<ThreeMarketTotalLine, number>();
  for (const line of available) {
    blendedTargets.set(
      line,
      blendBinaryInLogitSpace(
        overProbability(baseGrid, line),
        input.targets[line],
        input.directWeight,
      ),
    );
  ...
  for (let step = 0; step <= 160; step += 1) {
    const scale = 0.65 + step * 0.005;
    const total = (input.homeExpectedGoals + input.awayExpectedGoals) * scale;
    const away = total / (1 + ratio);
    const home = total - away;
    const grid = buildScoreGrid({ homeExpectedGoals: home, awayExpectedGoals: away, rho: input.rho, maximumGoals: input.maximumGoals }).scoreGrid;
    let loss = 0;
    for (const line of available) {
      const target = blendedTargets.get(line) ?? 0.5;
      const predicted = overProbability(grid, line);
      const lineWeight = line === 2.5 ? 1.2 : 1;
      loss += lineWeight * (logit(predicted) - logit(target)) ** 2;
    }
    if (loss < bestLoss) { bestLoss = loss; bestScale = scale; }
  }
  return {
    homeExpectedGoals: clamp(input.homeExpectedGoals * bestScale, 0.05, 6),
    awayExpectedGoals: clamp(input.awayExpectedGoals * bestScale, 0.05, 6),
  };
}
...
export function buildThreeMarketProjection(
  ...
  const calibratedGoals = calibrateExpectedGoalsToTotals({
    homeExpectedGoals: safeHome,
    awayExpectedGoals: safeAway,
    rho,
    maximumGoals,
    targets: {
      ...(Number.isFinite(input.directModel?.over15) ? { 1.5: input.directModel?.over15 as number } : {}),
      ...(Number.isFinite(input.directModel?.over25) ? { 2.5: input.directModel?.over25 as number } : {}),
      ...(Number.isFinite(input.directModel?.over35) ? { 3.5: input.directModel?.over35 as number } : {}),
    },
  ...
  const totals = Object.fromEntries(
    THREE_MARKET_TOTAL_LINES.map((line) => {
      ...
      const pair = makeProbabilityPair({
        ...
        uncertainty: clamp(
          line === 1.5
            ? input.uncertainty?.over15 ?? 0
            : line === 2.5
              ? input.uncertainty?.over25 ?? 0
              : input.uncertainty?.over35 ?? 0,
          0,
          0.25,
        ),
        classCount: 2,
