...
export interface MarketComparisonMetrics {
  ...
  accuracySkillVsMajority: number | null;
  prevalenceBaselineBrier: number | null;
  prevalenceBaselineLogLoss: number | null;
  brierSkillVsPrevalence: number | null;
  logLossSkillVsPrevalence: number | null;
...
function binaryMajorityBaseline(rows: Array<{ actualPositive: boolean }>): number | null {
  if (rows.length === 0) return null;
  const positives = rows.filter((row) => row.actualPositive).length;
  return Math.max(positives, rows.length - positives) / rows.length;
}

function hdaMajorityBaseline(rows: Array<{ actualClass: 'HOME' | 'DRAW' | 'AWAY' }>): number | null {
  if (rows.length === 0) return null;
  const counts = { HOME: 0, DRAW: 0, AWAY: 0 };
  for (const row of rows) counts[row.actualClass] += 1;
  return Math.max(counts.HOME, counts.DRAW, counts.AWAY) / rows.length;
}

function binaryProperScoreBaseline(rows: Array<{ actualPositive: boolean }>): {
  brier: number | null;
  logLoss: number | null;
} {
  if (rows.length === 0) return { brier: null, logLoss: null };
  const positives = rows.filter((row) => row.actualPositive).length;
  const p = Math.min(1 - 1e-9, Math.max(1e-9, positives / rows.length));
  return {
    brier: p * (1 - p),
...
function hdaProperScoreBaseline(rows: Array<{ actualClass: 'HOME' | 'DRAW' | 'AWAY' }>): {
  brier: number | null;
  logLoss: number | null;
} {
  if (rows.length === 0) return { brier: null, logLoss: null };
  ...
  return {
    brier: 1 - (p.HOME ** 2 + p.DRAW ** 2 + p.AWAY ** 2),
    logLoss: -(['HOME', 'DRAW', 'AWAY'] as const).reduce(
      (sum, key) => sum + p[key] * Math.log(Math.max(1e-9, p[key])),
      0,
    ),
  };
}

function properScoreSkill(candidate: number | null, baseline: number | null): number | null {
  if (candidate == null || baseline == null || baseline <= 1e-12) return null;
...
function comparison(
  ...
  majorityBaselineAccuracy: number | null,
  properScoreBaseline: { brier: number | null; logLoss: number | null },
): MarketComparisonMetrics {
  return {
    candidate,
    majorityBaselineAccuracy,
    accuracySkillVsMajority:
      candidate.accuracy == null || majorityBaselineAccuracy == null
        ? null
        : candidate.accuracy - majorityBaselineAccuracy,
    prevalenceBaselineBrier: properScoreBaseline.brier,
    prevalenceBaselineLogLoss: properScoreBaseline.logLoss,
    brierSkillVsPrevalence: properScoreSkill(candidate.brier, properScoreBaseline.brier),
    logLossSkillVsPrevalence: properScoreSkill(candidate.logLoss, properScoreBaseline.logLoss),
    champion,
    deltaVsChampion:
      champion == null
        ? null
...
export function selectBestEvaluatedMarket(
  ...
  for (const horizon of horizons) {
    for (const market of Object.keys(horizon.markets) as ThreeMarketEvaluationMarket[]) {
      ...
      const comparison = horizon.markets[market];
      if (
        selective.releasedRows < minimumSelectiveRows ||
        selective.coverage == null ||
        selective.hitRate == null ||
        selective.wilsonLower95 == null ||
        comparison.brierSkillVsPrevalence == null ||
        comparison.logLossSkillVsPrevalence == null ||
        comparison.brierSkillVsPrevalence <= 0 ||
        comparison.logLossSkillVsPrevalence <= 0
      ) continue;
      candidates.push({
        market,
    ...
    horizonSummaries.push({
      ...
      markets: {
        HDA: comparison(
          ...
          state.selective.HDA,
          hdaMajorityBaseline(state.hda),
          hdaProperScoreBaseline(state.hda),
        ),
        'O1.5': comparison(
          evaluateBinaryPredictions(state.totals[1.5]),
          null,
          state.selective['O1.5'],
          binaryMajorityBaseline(state.totals[1.5]),
          binaryProperScoreBaseline(state.totals[1.5]),
        ),
        'O2.5': comparison(
          evaluateBinaryPredictions(state.totals[2.5]),
          evaluateBinaryPredictions(state.championOver25),
          state.selective['O2.5'],
          binaryMajorityBaseline(state.totals[2.5]),
          binaryProperScoreBaseline(state.totals[2.5]),
        ),
        'O3.5': comparison(
          evaluateBinaryPredictions(state.totals[3.5]),
          null,
          state.selective['O3.5'],
          binaryMajorityBaseline(state.totals[3.5]),
          binaryProperScoreBaseline(state.totals[3.5]),
        ),
        BTTS: comparison(
          evaluateBinaryPredictions(state.btts),
          evaluateBinaryPredictions(state.championBtts),
          state.selective.BTTS,
          binaryMajorityBaseline(state.btts),
          binaryProperScoreBaseline(state.btts),
        ),
      },
    });
  }
