...
interface HistoricalExternalDecisionOddsRow {
  id: number;
...
interface HistoricalExternalClosingOddsRow {
  decimalOdds: number;
  sourceUpdatedAt: Date | null;
  sourceSnapshotAt: Date;
}
...
async function decisionOdds(input: {
  ...
  const apiRows = (await prisma.apiFootballOddsSnapshot.findMany({
    where: {
  ...
  const external = (await prisma.historicalExternalOddsSnapshot.findMany({
    where: {
      ...
      pitUsable: true,
      marketType: 'MATCH_WINNER',
      sourceSnapshotAt: { gte: cutoff, lte: input.decisionAsOf },
      kickoffAt: { gt: input.decisionAsOf },
    },
    orderBy: [{ sourceSnapshotAt: 'desc' }, { id: 'desc' }],
    select: {
      id: true,
      providerFixtureId: true,
      sourceUpdatedAt: true,
  ...
  })) as HistoricalExternalDecisionOddsRow[];

  const externalRows: LiveOddsRow[] = external.map((row) => ({
    id: -row.id,
    providerFixtureId: row.providerFixtureId,
    sourceUpdatedAt: row.sourceUpdatedAt,
    // Keep physical ingestion time for the late-ingestion audit. PIT eligibility
    // still uses sourceUpdatedAt (provider time), never this ingestion timestamp.
    observedAt: row.observedAt,
    bookmakerId: row.bookmakerNamespaceId,
    bookmakerName: `THE_ODDS_API::${row.bookmakerName}`,
    marketType: 'MATCH_WINNER',
    selection: row.selection as LiveOddsRow['selection'],
    lineValue: row.lineValue,
    decimalOdds: row.decimalOdds,
  }));

  return [...apiRows, ...externalRows].filter((row) => {
...
async function closingOdds(input: {
  ...
  if (input.bookmakerId < 0) {
    const rows = (await prisma.historicalExternalOddsSnapshot.findMany({
      where: {
        providerFixtureId: input.providerFixtureId,
        bookmakerNamespaceId: input.bookmakerId,
        marketType: 'MATCH_WINNER',
        selection: input.selection,
        pitUsable: true,
        sourceSnapshotAt: { gte: input.decisionAsOf, lt: input.kickoffAt },
      },
      orderBy: [{ sourceSnapshotAt: 'desc' }, { id: 'desc' }],
      take: 50,
      select: {
    ...
    })) as HistoricalExternalClosingOddsRow[];
    const valid = rows
      .filter((row) => (row.sourceUpdatedAt ?? row.sourceSnapshotAt).getTime() < input.kickoffAt.getTime())
      .filter((row) => Number.isFinite(row.decimalOdds) && row.decimalOdds > 1)
      .sort(
        (left, right) =>
          (right.sourceUpdatedAt ?? right.sourceSnapshotAt).getTime() -
          (left.sourceUpdatedAt ?? left.sourceSnapshotAt).getTime(),
      );
