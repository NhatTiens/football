...
} from './current-recommendation-risk-ranking.js';
import {
  buildPaperShadowRecommendation,
  type PaperShadowRecommendationDecision,
} from './paper-shadow-recommendation-core.js';
import {
...
export type CurrentRecommendationConfidenceTier = 'HIGH' | 'MEDIUM' | 'LIMITED';
...
export interface CurrentScientificCandidate {
  ...
  rawSignalEligibleBeforeRiskGuard: boolean;
  rawSignalRejectionReasons: string[];
  rankingVersion: string;
  signalTier: 'RISK_ADJUSTED_RESEARCH' | 'LOW_CONFIDENCE_DIAGNOSTIC';
...
export interface CurrentScientificRecommendationAnalysis {
  ...
  paperShadowRecommendation: PaperShadowRecommendationDecision;
  paperHdaContext: PaperHdaContextAdjustmentEngineResult | null;
...
function weightsRecord(value: unknown): Record<string, number> {
  ...
  return Object.fromEntries(
    Object.entries(value as Record<string, unknown>)
      ...
      .filter(([, item]): boolean => Number.isFinite(item)),
  );
}
...
function validMapping(provider: ProviderFixtureRow, local: LocalFixtureRow): boolean {
...
function currentHorizonMinutes(kickoffAt: Date, now: Date): number {
  const minutes = (kickoffAt.getTime() - now.getTime()) / 60_000;

  return Math.max(1, Math.round(minutes));
}

async function loadFrozenT90Registry(now: Date): Promise<FrozenRegistryRow | null> {
  return (await prisma.scientificCandidateRegistry.findFirst({
    where: {
...
function reliabilityGates(input: {
  ...
  for (const market of markets) {
    const historical = input.report.reliability[market];
    ...
    const horizonValidated =
      market === 'MATCH_WINNER' && input.horizonMinutes === 90 && input.frozenRegistryAvailable;

    result[market] = {
      market,
      status: horizonValidated
        ? historical.status
        : market === 'MATCH_WINNER' && input.horizonMinutes !== 90
...
function confidenceTier(input: {
  ...
  if (input.dataQualityScore >= 0.65 && input.historySampleSize >= 8) {
    return 'HIGH';
  }

  if (input.dataQualityScore >= 0.4 && input.historySampleSize >= 3) {
    return 'MEDIUM';
  }
...
async function buildCurrentModel(input: {
  ...
  const baselineOver25 = {
    OVER: baseline.over25.OVER,
    UNDER: baseline.over25.UNDER,
  };

  const baselineBtts = {
    YES: baseline.btts.YES,
    NO: baseline.btts.NO,
  };

  const modelConfidenceTier = confidenceTier({
    dataQualityScore: baseline.dataQualityScore,
    historySampleSize: baseline.historySampleSize,
  });

  if (fundamentals.available && fundamentals.dixonColes != null) {
    if (fundamentals.dixonColes.trainedThrough.getTime() > input.now.getTime()) {
      throw new Error('CURRENT_RECOMMENDATION_PIT_VIOLATION');
    }
    ...
    let hdaVersion = SCIENTIFIC_MODEL_VERSION;

    if (input.horizonMinutes === 90 && input.frozenRegistry != null) {
      ...
      hdaVersion = input.frozenRegistry.candidateVersion;
    }
  ...
  const paperHdaContext = await getPaperHdaContextAdjustment({
    fixtureId: input.localFixture.id,
    leagueId: input.localFixture.leagueId,
    homeTeamId: input.localFixture.homeTeamId,
    awayTeamId: input.localFixture.awayTeamId,
    predictionAsOf: input.now,
    baseline: baselineHda,
  ...
  return {
    ...
    modelFallbackReason: 'NO_DIXON_COLES_MODEL_AT_CURRENT_AS_OF',
    modelConfidenceTier,
    historySampleSize: baseline.historySampleSize,
    dataQualityScore: baseline.dataQualityScore,
...
function currentSignalAssessment(input: {
  ...
  if (input.edge < SCIENTIFIC_BEST_BET_POLICY.minimumEdge) {
    reasons.push('CURRENT_EDGE_BELOW_MINIMUM');
  }

  if (input.expectedValue < SCIENTIFIC_BEST_BET_POLICY.minimumExpectedValue) {
    reasons.push('CURRENT_EV_BELOW_MINIMUM');
  }
...
function compareCurrentCandidates(
  ...
  return compareRiskAdjustedCurrentCandidates(left, right);
}
...
function emptyAnalysis(input: {
  ...
  horizonMinutes: number;
  now: Date;
  status: CurrentScientificRecommendationAnalysis['status'];
  error?: string | null;
  oddsFreshness?: OddsFreshnessSummary;
}): CurrentScientificRecommendationAnalysis {
  return {
    version: CURRENT_SCIENTIFIC_RECOMMENDATION_VERSION,
    ...
    status: input.status,
    recommendation: null,
    paperShadowRecommendation: buildPaperShadowRecommendation({
      providerFixtureId: input.providerFixtureId,
      horizonMinutes: input.horizonMinutes,
      calculatedAt: input.now.toISOString(),
      candidates: [],
    ...
    oddsFreshness:
      ...
      emptyOddsFreshnessSummary(
        ...
        envNumber('CURRENT_RECOMMENDATION_MAX_ODDS_AGE_MINUTES', 360),
      ),
    marketCoverage: [],
...
// R410255_MISSING_PROVIDER_SNAPSHOT_TERMINAL_CLASSIFICATION
export function buildNoProviderFixtureSnapshotAnalysis(input: {
  providerFixtureId: number;
  kickoffAt: Date;
  now: Date;
  localFixtureId?: number | null;
  ...
  return emptyAnalysis({
    ...
    horizonMinutes: currentHorizonMinutes(input.kickoffAt, input.now),
    now: input.now,
    status: 'NO_PROVIDER_FIXTURE_SNAPSHOT',
    error: 'NO_PROVIDER_FIXTURE_SNAPSHOT_AT_CURRENT_AS_OF',
  });
}

export async function getCurrentScientificRecommendationMap(input: {
  providerFixtureIds: number[];
  now?: Date;
  maxFixtures?: number;
  maxOddsAgeMinutes?: number;
}): Promise<Map<number, CurrentScientificRecommendationAnalysis>> {
  ...
  const maxFixtures = Math.max(
    ...
    Math.min(
      120,
      Math.floor(input.maxFixtures ?? envNumber('CURRENT_RECOMMENDATION_MAX_FIXTURES', 60)),
    ),
  );

  const maxOddsAgeMinutes = Math.max(
    1,
    input.maxOddsAgeMinutes ?? envNumber('CURRENT_RECOMMENDATION_MAX_ODDS_AGE_MINUTES', 360),
  );
  ...
  const result = new Map<number, CurrentScientificRecommendationAnalysis>();
  ...
  const providers = latestProviderFixtures(providerRows)
    .sort(
      (left: ProviderFixtureRow, right: ProviderFixtureRow): number =>
        left.kickoffAt.getTime() - right.kickoffAt.getTime(),
    )
    .slice(0, maxFixtures);

  const ids = providers.map((row: ProviderFixtureRow): number => row.providerFixtureId);

  if (ids.length === 0) {
    return result;
  }

  const oddsLookbackDays = Math.max(
    1,
    Math.min(30, Math.floor(envNumber('CURRENT_RECOMMENDATION_ODDS_LOOKBACK_DAYS', 14))),
  );

  const oddsLookbackCutoff = new Date(now.getTime() - oddsLookbackDays * 86_400_000);

  const [localRowsRaw, oddsRowsRaw, reliabilityReport] = await Promise.all([
  ...
  const localByProvider = new Map<number, LocalFixtureRow>(
    localRows.map((row: LocalFixtureRow): [number, LocalFixtureRow] => [row.apiFixtureId, row]),
  );

  const oddsFreshnessResolution = await resolveFreshReobservedOdds({
    providerFixtureIds: ids,
    oddsRows: oddsRowsRaw as ReobservationOddsRow[],
    now,
    maximumAgeMinutes: maxOddsAgeMinutes,
  });

  const oddsByProvider = new Map<number, LiveOddsRow[]>();

  for (const row of oddsFreshnessResolution.activeOddsRows) {
    const rows = oddsByProvider.get(row.providerFixtureId) ?? [];

    rows.push(row);

    oddsByProvider.set(row.providerFixtureId, rows);
  }

  let frozenRegistry: FrozenRegistryRow | null | undefined;

  for (const provider of providers) {
    const horizonMinutes = currentHorizonMinutes(provider.kickoffAt, now);

    const oddsFreshness =
      oddsFreshnessResolution.summaryByFixture.get(provider.providerFixtureId) ??
      emptyOddsFreshnessSummary(provider.providerFixtureId, maxOddsAgeMinutes);

    const local = localByProvider.get(provider.providerFixtureId);
    ...
    if (local == null) {
      result.set(
        ...
        emptyAnalysis({
          ...
          now,
          oddsFreshness,
          status: 'UNMAPPED_FIXTURE',
        }),
      );
      continue;
    }

    if (!validMapping(provider, local)) {
      result.set(
        provider.providerFixtureId,
        emptyAnalysis({
          providerFixtureId: provider.providerFixtureId,
    ...
    if (fixtureOdds.length === 0) {
      result.set(
        ...
        emptyAnalysis({
          ...
          now,
          oddsFreshness,
          status: 'NO_FRESH_PIT_ODDS',
        }),
      );
      continue;
    }

    if (horizonMinutes === 90 && frozenRegistry === undefined) {
      frozenRegistry = await loadFrozenT90Registry(now);
    }

    try {
      const model = await buildCurrentModel({
        localFixture: local,
        now,
        horizonMinutes,
        frozenRegistry: horizonMinutes === 90 ? (frozenRegistry ?? null) : null,
      });

      const gates = reliabilityGates({
        horizonMinutes,
        frozenRegistryAvailable: horizonMinutes !== 90 || frozenRegistry != null,
      ...
      if (candidateSet.candidates.length === 0) {
        result.set(provider.providerFixtureId, {
          ...emptyAnalysis({
            providerFixtureId: provider.providerFixtureId,
            localFixtureId: local.id,
            kickoffAt: provider.kickoffAt,
      ...
      const quoteConsensusBySelection = buildCurrentQuoteConsensusMap(
        fixtureOdds.map((row: LiveOddsRow) => ({
      ...
      const candidates = candidateSet.candidates
        .map((candidate): CurrentScientificCandidate => {
          const normalized = normalizePaperBetCandidate(candidate);

          const scientific = normalized.scientificCandidate;

          const official = assessBestBetCandidate(scientific);

          const officialEligible = official.eligible && model.officialPromotionAllowed;

          const officialRejectionReasons = model.officialPromotionAllowed
            ? official.rejectionReasons
          ...
          const riskAssessment = assessRiskAdjustedCurrentCandidate({
            ...quote,
            modelProbability: candidate.modelProbability,
            fairMarketProbability: candidate.fairMarketProbability,
            rawEdge: scientific.edge,
            rawExpectedValue: scientific.expectedValue,
            reliabilityStatus: candidate.reliabilityStatus,
            modelSource: model.modelSource,
            confidenceTier: model.modelConfidenceTier,
            historySampleSize: model.historySampleSize,
            dataQualityScore: model.dataQualityScore,
          ...
          const freshnessEvidence: OddsFreshnessEvidence | null =
            candidate.sourceOddsSnapshotId == null
              ? null
              : (oddsFreshnessResolution.evidenceBySnapshotId.get(candidate.sourceOddsSnapshotId) ??
                null);
          ...
          return {
            ...
            edge: scientific.edge,
            expectedValue: scientific.expectedValue,
            rawSignalEligibleBeforeRiskGuard: rawCurrentSignal.eligible,
            rawSignalRejectionReasons: rawCurrentSignal.reasons,
            rankingVersion: riskAssessment.rankingVersion,
            signalTier: riskAssessment.signalTier,
            effectiveModelWeight: riskAssessment.effectiveModelWeight,
            adjustedModelProbability: riskAssessment.adjustedModelProbability,
            probabilityHaircut: riskAssessment.probabilityHaircut,
            conservativeProbability: riskAssessment.conservativeProbability,
            conservativeEdge: riskAssessment.conservativeEdge,
            conservativeExpectedValue: riskAssessment.conservativeExpectedValue,
            longshotPenalty: riskAssessment.longshotPenalty,
            riskAdjustedScore: riskAssessment.riskAdjustedScore,
            quoteCount: riskAssessment.quoteCount,
            quoteMedianOdds: riskAssessment.quoteMedianOdds,
            quoteAgreementRatio: riskAssessment.quoteAgreementRatio,
            quoteDeviationRatio: riskAssessment.quoteDeviationRatio,
            quoteOutlier: riskAssessment.quoteOutlier,
            reliabilityStatus: candidate.reliabilityStatus,
            modelSource: model.modelSource,
            modelFallbackReason: model.modelFallbackReason,
            modelConfidenceTier: model.modelConfidenceTier,
            modelHistorySampleSize: model.historySampleSize,
            modelDataQualityScore: model.dataQualityScore,
            currentSignalEligible: riskAssessment.eligible,
            currentSignalRejectionReasons: riskAssessment.rejectionReasons,
            officialEligible,
            officialRejectionReasons,
            sourceOddsSnapshotId: candidate.sourceOddsSnapshotId,
            sourceOddsEffectiveAt:
              (candidate.sourceOddsUpdatedAt ?? candidate.sourceOddsObservedAt)?.toISOString() ??
              null,
            sourceOddsUpdatedAt: candidate.sourceOddsUpdatedAt?.toISOString() ?? null,
            sourceOddsFirstObservedAt: candidate.sourceOddsObservedAt?.toISOString() ?? null,
            sourceOddsReobservedAt: freshnessEvidence?.reobservedAt?.toISOString() ?? null,
            sourceOddsFreshnessAt: freshnessEvidence?.freshnessAt.toISOString() ?? null,
      ...
      const selected =
        candidates.find(
          (candidate: CurrentScientificCandidate): boolean => candidate.currentSignalEligible,
        ) ?? null;
      ...
      const recommendation =
        selected == null
          ? null
          : {
              source: 'CURRENT_SCIENTIFIC_PIT' as const,
              version: CURRENT_SCIENTIFIC_RECOMMENDATION_VERSION,
              ...
              bookmakerName: selected.bookmakerName,
              modelProbability: selected.modelProbability,
              fairMarketProbability: selected.fairMarketProbability,
              edge: selected.edge,
              expectedValue: selected.expectedValue,
              rankingVersion: selected.rankingVersion,
              signalTier: 'RISK_ADJUSTED_RESEARCH' as const,
              effectiveModelWeight: selected.effectiveModelWeight,
              adjustedModelProbability: selected.adjustedModelProbability,
              probabilityHaircut: selected.probabilityHaircut,
              conservativeProbability: selected.conservativeProbability,
              conservativeEdge: selected.conservativeEdge,
              conservativeExpectedValue: selected.conservativeExpectedValue,
              longshotPenalty: selected.longshotPenalty,
              riskAdjustedScore: selected.riskAdjustedScore,
              quoteCount: selected.quoteCount,
              quoteMedianOdds: selected.quoteMedianOdds,
              quoteAgreementRatio: selected.quoteAgreementRatio,
              quoteDeviationRatio: selected.quoteDeviationRatio,
              quoteOutlier: false as const,
              reliabilityStatus: selected.reliabilityStatus,
              modelSource: selected.modelSource,
              modelFallbackReason: selected.modelFallbackReason,
              modelConfidenceTier: selected.modelConfidenceTier,
              modelHistorySampleSize: selected.modelHistorySampleSize,
              currentSignalEligible: true as const,
              officialEligible: selected.officialEligible,
              dataQualityScore: model.dataQualityScore,
              modelVersion: model.modelVersion,
              sourceOddsSnapshotId: selected.sourceOddsSnapshotId,
              sourceOddsEffectiveAt: selected.sourceOddsEffectiveAt,
              sourceOddsUpdatedAt: selected.sourceOddsUpdatedAt,
              sourceOddsFirstObservedAt: selected.sourceOddsFirstObservedAt,
              sourceOddsReobservedAt: selected.sourceOddsReobservedAt,
              sourceOddsFreshnessAt: selected.sourceOddsFreshnessAt,
      ...
      result.set(provider.providerFixtureId, {
        ...
        horizonMinutes,
        calculatedAt: now.toISOString(),
        status: recommendation == null ? 'NO_VALUE_SIGNAL' : 'AVAILABLE',
        recommendation,
        paperShadowRecommendation,
        paperHdaContext: model.paperHdaContext,
        candidates,
        oddsFreshness,
        marketCoverage: candidateSet.marketCoverage,
        error: null,
...
export async function getCurrentScientificRecommendation(input: {
  providerFixtureId: number;
  now?: Date;
  maxOddsAgeMinutes?: number;
  ...
  const map = await getCurrentScientificRecommendationMap({
    ...
    now: input.now,
    maxFixtures: 1,
    maxOddsAgeMinutes: input.maxOddsAgeMinutes,
  });
